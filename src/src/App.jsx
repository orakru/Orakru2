...
const Chips = ({ opts, val, set, multi }) => (
  <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 20 }}>
    {opts.map(o => {
      ...
      return (
        <button key={o} onClick={() => multi ? set(on ? (val || []).filter(x => x !== o) : [...(val || []), o]) : set(on ? "" : o)}
          style={{ padding: "7px 14px", background: on ? "#0A0A0A" : "transparent", color: on ? "#fff" : "#0A0A0A", border: `1.5px solid ${on ? "#0A0A0A" : BD}`, fontSize: 11, cursor: "pointer", fontFamily: sans }}>
          {o}
        </button>
      );
    })}
...
export default function App() {
  ...
  const doAuth = async () => {
    ...
    try {
      ...
      if (d.error || d.error_description) {
        setErr(d.error_description || d.error?.message || "Error");
      } else {
  ...
  const proSteps = [
    { title: "Let's start with the basics.", valid: pro.name && pro.email, fields: <><Field label="Your Name" val={pro.name} set={v => sp("name", v)} /><Field label="Business Name (optional)" val={pro.business_name} set={v => sp("business_name", v)} /><Field label="Email" val={pro.email} set={v => sp("email", v)} type="email" /><Field label="Instagram (optional)" val={pro.instagram} set={v => sp("instagram", v)} ph="@handle" /></> },
    { title: "Where are you based?", valid: pro.city && pro.zip && pro.setting, fields: <><Field label="City" val={pro.city} set={v => sp("city", v)} /><Field label="Zip Code" val={pro.zip} set={v => sp("zip", v)} /><Lbl t="Your Setting" /><Chips opts={SET} val={pro.setting} set={v => sp("setting", v)} /></> },
    ...
    { title: "Almost done.", valid: pro.about_me.length > 0, fields: <><Field label="About Me" val={pro.about_me} set={v => sp("about_me", v)} ph="Your story, your specialty, what makes working with you different..." rows={5} max={300} /><Field label="Ideal Client (optional)" val={pro.preferred_client} set={v => sp("preferred_client", v)} ph="Who you work best with..." rows={3} /></> },
  ];

  const clientSteps = [
    { title: "Nice to meet you.", valid: cli.name && cli.email, fields: <><Field label="Your Name" val={cli.name} set={v => sc("name", v)} /><Field label="Email" val={cli.email} set={v => sc("email", v)} type="email" /></> },
    { title: "Where are you located?", valid: cli.city && cli.zip, fields: <><Field label="City" val={cli.city} set={v => sc("city", v)} /><Field label="Zip Code" val={cli.zip} set={v => sc("zip", v)} /></> },
    ...
    { title: "Tell us about yourself.", valid: cli.about_me.length > 0, fields: <><Field label="About Me" val={cli.about_me} set={v => sc("about_me", v)} ph="Who you are, what you've been putting up with, what you actually want..." rows={5} max={300} /></> },
  ];
  ...
  if (scr === "auth") return (
    <Wrap>
      ...
      <Field label="Email" val={em} set={setEm} type="email" ph="your@email.com" />
      <Field label="Password" val={pw} set={setPw} type="password" ph="at least 6 characters" />
